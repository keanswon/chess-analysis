...
export function StockfishEval({ 
  ...
  const getEvaluationColorClass = (evaluation: number | string | null): string => {
    ...
    if (typeof evaluation === 'string' && evaluation.startsWith('M')) {
      return evaluation.includes('-') ? 'text-red-400' : 'text-green-400';
    }
    
    const numEval = parseFloat(evaluation.toString());
    if (isNaN(numEval)) return 'text-white';
    
    if (numEval > 0) return 'text-green-400';
    if (numEval < 0) return 'text-red-400';
    return 'text-white';
  };
  ...
  if (loading) {
    return (
      <div className={`stockfish-eval ${className}`}>
        <div className="animate-pulse bg-gray-800 rounded-xl p-4 border border-gray-700 min-w-72">
  ...
  return (
    <div className={`stockfish-eval ${className}`}>
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-4 border border-gray-700 shadow-lg min-w-72">
        {/* Header */}
        <div className="flex justify-between items-center mb-3">
          <div className="flex items-center gap-2">
